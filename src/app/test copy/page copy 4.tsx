...
const Navbar = () => {
  return (
    <div
      className="flex align-center sticky top-0 z-1000 px-20 bg-white shadow-2"
      ...
      <div className="mt-20 bg-grey-100 b-1 b-grey-300">
        ...
        >
          {Array.from({ length: 5 }, (_, i) => (
            <a
              key={i}
              href={`#${idPrefix}sub${i + 1}`}
              className="fs-14 fw-500 hover:underline"
            >
              Sub-Link {i + 1}
            </a>
          ))}
        ...
        <div className="flex flex-row p-20 fs-16 sx-20 overflow-x-auto">
          {Array.from({ length: 5 }, (_, i) => (
            <div
              key={i}
              id={idPrefix + "sub" + (i + 1)}
              className="flex-0 w-2/3 bg-white r-20 p-20 sy-10"
            >
